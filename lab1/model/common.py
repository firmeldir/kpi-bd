...
class Model:
    ...
    def truncate_tables(self):
        self.__cursor.execute('TRUNCATE ONLY goods, '
                              'orders, warehouses RESTART IDENTITY CASCADE')
        self.__connection.commit()

    def drop_tables(self):
        self.__cursor.execute('DROP TABLE IF EXISTS '
                              'goods, orders, warehouses CASCADE')
        self.__connection.commit()

    def filter_items(self, cost_from: int, cost_to: int, sender_name: str = None, recipient_name: str = None):
        query = "SELECT order_id, recipients_date, order_date, shipping_cost, c1.name, " \
                "c1.phone_number, c2.name, c2.phone_number from orders i " \
