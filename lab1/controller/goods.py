...
class GoodsController(BaseController):
    ...
    def _prompt_values_for_input(item: object = None, for_update: bool = False):
        prompts = ['Height (mm)', 'Width (mm)', 'Depth (mm)',
                   'Weight (g)', 'Description (could be empty)', 'Order id']
        values = [item.height, item.width, item.depth, item.weight, item.description,
                  item.order_id] if isinstance(item, Goods) else None
        return prompts, values

    @staticmethod
    def _create_obj_from_input(input_items: [dict]):
        height = width = depth = weight = description = order_id = None
        for item in input_items:
            ...
            elif item['name'] == 'Order id':
                order_id = int(item['value'])
        return Goods(height, width, depth, weight, order_id, description)
