...
from controller.orders import OrdersController, Orders
...
class Controller:
    def __init__(self, connection, program_name):
        ...
        self.__orders_controller = OrdersController(connection, self.__common_view)
    ...
    def crud_operations(self):
        list_menu = ['Contragents', 'Warehouses', 'Orders', 'Goods']
        ...
        if menu_option == 0:
            ...
            self.__orders_controller.choose_operation(self.crud_operations)
        elif menu_option == 3:
    ...
    def batch_generation_data(self):
        try:
            ...
            warehouses = self.__random_warehouses(n)
            self.__warehouse_controller.model.create_many(warehouses, True)
            orders = self.__random_orders(contragents, warehouses, n)
            self.__orders_controller.model.create_many(orders, True)
            goods = self.__random_goods(orders, min(n, 10))
    ...
    def search_multiple_attr(self):
        try:
            min_cost, max_cost = self.__orders_controller.model.get_extremum_shipping_cost()
    ...
    @staticmethod
    def __random_warehouses(amount: int = 1):
        office_numbers = Controller.__random_phone_number(amount)
        cities = Controller.__random_cities(amount)
        return [Warehouse("city", office_number) for office_number
                in zip(office_numbers)]

    @staticmethod
    def __random_orders(contragents: [Contragent], warehouses: [Warehouse], amount: int = 1):
        ...
        while counter < amount:
            ...
            Controller.__append_uniq_elements(warehouse_dep_nums, warehouse_arr_nums, warehouses, 'warehouse_id')
            counter += 1
        return [Orders(date_dep, cost, send_ipn, recp_ipn, war_dep_num, date_arr)
    ...
    @staticmethod
    def __random_goods(orders: [Orders], amount_per_invoice: int = 1):
        orders_id = []
        for invoice in orders:
            counter = 0
            while counter < amount_per_invoice:
                orders_id.append(invoice.num)
                if random.randrange(20) % 4 == 1:
                    break
                counter += 1
        amount = len(orders_id)
        ...
        return [Goods(height, width, depth, weight, inv_id, description)
                ...
                in zip(heights, widths, depths, weights, descriptions, orders_id)]
    ...
    def __random_cities(amount: int = 1):
        ...
        for item in content['results']:
            ...
            city.append(f"{street['name']}, {street['number']}")
        return city
